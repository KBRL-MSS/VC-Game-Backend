...
public class WebSocketController {
    ...
    public void sendPartyInvite(Principal principal, @Payload PartyInviteRequest request) {
        ...
        boolean isOnlineFriend = onlineFriends.stream()
        ...
        if (!isOnlineFriend) {
            messagingTemplate.convertAndSendToUser(
                    senderId,
                    "/queue/error",
                    "User is not online or not in your friend list"
            );
            return;
        }

        try {
            PartyInvite invite = partyService.inviteToParty(
                    request.getPartyId(),
                    senderId,
            ...
            messagingTemplate.convertAndSendToUser(
                    request.getReceiverId(),
                    "/queue/party-invite",
                    new PartyInviteResponse(invite)
            );

            // Confirm to sender
            messagingTemplate.convertAndSendToUser(
                    senderId,
                    "/queue/party-invite-sent",
                    "Invite sent to player"
            );
        } catch (Exception e) {
            messagingTemplate.convertAndSendToUser(
                    senderId,
            ...
            try {
                ...
                for (String memberId : updatedParty.getMembers()) {
                    messagingTemplate.convertAndSendToUser(
                            memberId,
                            "/queue/party-update",
                            new PartyResponse(updatedParty)
                    );
                }
