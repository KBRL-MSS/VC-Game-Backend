...
public class PartyController {
  ...
  @GetMapping
  public ResponseEntity<List<PartyDto>> getUserParties(
    ...
    List<PartyDto> partyDtos =
        parties.stream().map(this::convertToDto).collect(Collectors.toList());

    return ResponseEntity.ok(partyDtos);
  }

  @GetMapping("/invites")
  public ResponseEntity<List<PartyInviteDto>> getPendingInvites(
      @AuthenticationPrincipal UserDetails userDetails) {
    User user =
        userRepository
            .findByUsername(userDetails.getUsername())
    ...
    List<PartyInviteDto> inviteDtos =
        invites
            .stream()
            .map(
                invite -> {
  ...
  public ResponseEntity<PartyDto> getParty(
      @AuthenticationPrincipal UserDetails userDetails, @PathVariable String partyId) {
